...
abstract: "Air transportation is undergoing a fundamental transformation. On the political,
  ...
  for the arrival phase. This paper applies the concept and approach as a use case
  analysis for 2 European airports with different strategies for sequencing arrival
  traffic (i.e. stack holding and point merge).\nThe research reported demonstrates the
  feasibility of the reproducibility approach. This work introduces a novel workflow by making available a
  companion web repository that provides data, code, and instructions to fully reproduce the
  paper.\n"
title-meta: Towards Reproducibility in ANS Performance
---
...
Similar practise is reported in other domains, e.g. medicine [@kolata_2011], psychology [@ritchie_2012], ecology [@fraser_etal_2018]. Fidler and Wilcox [@fidler_wilcox_2018] provides a recent review of the reproducibility discussion. 
...
With a view to enhance transparency, PRU is - inter alia - making performance-related data publicly available via its portal, [http://ansperformance.eu](http://ansperformance.eu), since early 2017 combined with the methodological documentation for the calculation of the performance indicators used under the EUROCONTROL Performance Review System and the European Union Performance Scheme.
...
This can be contrasted by emerging practices like data journalism. One interesting example is buzzFeed's article on the use of surveillance planes operated by law enforcement and military. The authors were able to demonstrate based on Open Data that these purposes range from tracking drug trafficking to testing new surveillance technology [@peteraldhous_2016]. The credibility of the article comes from the fact that all artefacts are made available to reproduce the findings or study the approach and data transformations leading to the conclusions of the article.
...
* engineering / signal processing: Vandewalle [@vandewalle_2009] discuss the motivation and approach to reproducibility in signal processing (i.e. what, why, and how). Also here the case is made to open access to the code and data for validation purposes .
* operational ANS Performance monitoring: Spinielli et al. [@spinielli_2018] highlight the development and use of Open Flight Trajectories for reproducible ANS .
...
The work of Marwick et al. [@marwick_2017] and Peng [@Peng1226] uses the analogy of a spectrum that can be mapped against the scoring of Vandewalle [@vandewalle_2009] describing the level of reproducibility on a 0 to 5 scale [@marwick_2017].
...
The abstraction between source data and analytic data allows for the sharing of such data as limitations are typically only applicable to the source data. We follow the notion of [@marwick_2017] by referring to the provision of a representative sample of 8 days accompanying this paper as source data.  
...
- A code chunk starts with three backticks and `r` indicates the
  language name,^[R Markdown supports 50 other computer languages.] and ends with
  three backticks.
...
```{r rmd-example,echo=FALSE,fig.cap='(ref:rmd-example)',fig.pos='htb',fig.height=2}
...
This work has been reported in [@koelle_open_2017] and [@spinielli_2018]. The idea is to establish a pan-European air situation picture openly available to researchers and practitioners.
...
```{r data-preparation-pipeline,fig.cap='(ref:data-preparation-pipeline)',fig.pos='htb',fig.height=2.5}
...
have been extracted.
The following steps and scripts comprise [@performancereviewunit_2019]:

1. the script `R/extract-arrivals.R` extracts arrivals to the `APT` airport and generates files with:
...
ggplot(data = tmp
  ...
  theme(
    ...
    legend.position = c(0.95, 0.60), legend.justification = c(1, 0)
...
```{r ASMA-time-dist, fig.cap='(ref:ASMA-time-dist)', message = FALSE,fig.pos='hbt',fig.height=4}
...
knitr::kable(dist_asma, digits = 2
             ,caption = "Average additional distance flown (15-th pct)")
```
...
```{r egll-trajectory-example, fig.cap='(ref:egll-trajectory-example)',fig.pos='htb',fig.height=4}
...
```{r eidw-trajectory-example,fig.cap='(ref:eidw-trajectory-example)',fig.pos='bht',fig.height=6.5}
eidw_long_dist <- asma_dist2(eidw_pts, eidw_dist) %>% 
  filter(TRAVEL > 5) %>%    # remove outlier
  arrange(desc(ADD_DIST))
eidw_day1 <- readr::read_csv("data/eidw_pos_rt_40NM_2017-08-01.csv")

eidw_long_dist <- eidw_long_dist %>%
  mutate(IS_DAY1 = ifelse(flight_id %in% eidw_day1$flight_id, 1, 0)) %>%
  filter(IS_DAY1 == 1) %>%
  filter(ADD_DIST < 50) %>% 
...
ff <- "data/eidw_flt_rt_2017-08-01.csv" %>% 
  vroom::vroom()

pp40 <- "data/eidw_pos_rt_40NM_2017-08-01.csv" %>% 
  vroom::vroom() %>%
  group_by(flight_id) %>% 
  filter(flight_id %in% fids) %>% 
  dplyr::arrange(timestamp) %>%
  dplyr::left_join(ff, by = "flight_id")
...
eidw_holdings_sf_buf <- eidw_holdings_sf %>%
  st_transform(29902) %>%
  st_buffer(buf) %>%
  st_transform(4326)
...
ggplot2::ggplot(pp40) +
  ...
  ggplot2::geom_sf(data = eidw_holdings_sf_buf, alpha = 0) +
  grw_sf +
  gvor + gvorl +
...
```{r distance-flown-segment, fig.cap="(ref:distance-flown-segment)",fig.height=4}
...
```{r eidw-saturation-example, fig.cap='(ref:eidw-saturation-example)',fig.pos='hb'}
...
the establishment of the current performance reporting based on the trajectory (via [http://ansperformance.eu](http://ansperformance.eu), and setup of a network with interested researchers and professionals.
